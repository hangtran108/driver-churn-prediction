...
del driver_data['ops_city_id']
del driver_data['activated_date']
del driver_data['device_type']
del driver_data['ftt_time']
del driver_data['vehicle_brand']
del driver_data['vehicle_model']
del driver_data['segment']
...
driver_sparse = driver_data[['week']].drop_duplicates().merge(driver_data[['driver_id']].drop_duplicates(), how = 'cross')
driver_sparse = driver_sparse.merge(driver_data, how = 'left', on = ['week', 'driver_id'])
driver_sparse = driver_sparse.merge(driver_date, how = 'left',
                                    on = 'driver_id').query('week >= first_week')
del driver_date
del driver_sparse['first_week']
driver_sparse = driver_sparse.fillna(0).reset_index(drop = True)
driver_sparse
driver_sparse_merge = []
for week in driver_sparse['week'].unique()[:]:
    current = driver_sparse[driver_sparse['week'] == week][['week', 'driver_id']]
    current_past = driver_sparse[driver_sparse['week'] <= week][driver_sparse['week'] + pd.Timedelta(days = 14) >= week]
    del current_past['week']
    del current_past['not_churn']
    current = current.merge(current_past, how = 'left', on = 'driver_id').groupby(['week', 'driver_id']).mean().reset_index()
    driver_sparse_merge.append(current)
driver_final = pd.concat(driver_sparse_merge).reset_index(drop = True)
driver_final
driver_sparse = driver_sparse.sort_values(['week', 'driver_id']).reset_index(drop = True)
driver_data_self = driver_sparse[['week', 'driver_id', 'not_churn']]
...
driver_data_self
driver_data_self = driver_sparse[['week', 'driver_id', 'not_churn']].merge(driver_data_self, how = 'left', on = ['week', 'driver_id'])
# Since latest week has no future information, so we drop it
...
driver_final = driver_data_self.merge(driver_final, how = 'inner', on = ['week', 'driver_id'])
driver_final[driver_final['not_churn'] == 1]['not_churn_1w'].mean()
driver_data_self = driver_sparse[['week', 'driver_id', 'not_churn']]
driver_data_self = driver_data_self.merge(driver_data_self, how = 'left', on = 'driver_id', suffixes = ['', '_pre_1w'])
driver_data_self = driver_data_self[driver_data_self.week_pre_1w == driver_data_self.week - pd.Timedelta(days = 7)].reset_index(drop = True)
del driver_data_self['not_churn']
del driver_data_self['week_pre_1w']
driver_data_self
driver_data_self = driver_sparse[['week', 'driver_id']].merge(driver_data_self, how = 'left', on = ['week', 'driver_id'])
# Since first week has no past information, so we drop it
first_week = driver_data_self['week'].min()
driver_data_self = driver_data_self[driver_data_self['week'] != first_week].reset_index(drop = True)
# Now any null value of column 'not_churn_pre_1w' means that the driver disappear (not online) -> churn
driver_data_self['not_churn_pre_1w'] = driver_data_self['not_churn_pre_1w'].fillna(0)
driver_final = driver_data_self.merge(driver_final, how = 'inner', on = ['week', 'driver_id'])
def segment(sum_sh):
    if sum_sh < 10:
        return 'Part-time'
    ...
    if sum_sh < 60:
        return 'Full-time'
    return 'High Full-time'
driver_final['segment'] = driver_final['sum_sh'].progress_apply(lambda x: segment(x))
driver_final[driver_final['driver_id'] == 342]
# Split Train - Test
first_weeks = driver_final['week'].unique()[:2]
driver_final = driver_final[~driver_final['week'].isin(first_weeks)]
latest_week = driver_final['week'].max()
driver_data_test = driver_final[driver_final['week'] == latest_week].reset_index(drop = True)
driver_data_train = driver_final[driver_final['week'] != latest_week].reset_index(drop = True)
IPD(driver_data_train)
IPD(driver_data_test)
driver_data.describe()
driver_data_train.columns.values
driver_data.dtypes
driver_data_train.iloc[:, 15:-2].values
X = driver_data_train[['not_churn_pre_1w', 'not_churn', 'sum_sh', 'completed_trip',
...
def plot_by_woe(df_WoE, rotation_of_x_axis_labels = 0, figsize = (10, 5)):
    ...
    plt.show()
discrete_cols = ['not_churn_pre_1w',	'not_churn', 'segment']
...
df_discrete
selected_discrete_cols = ['not_churn_pre_1w',	'not_churn', 'segment']
...
train_data_Y
pd.options.display.max_rows = None
reg_1 = LogisticRegression_with_p_values()
reg_1.fit(train_data_X, train_data_Y)
summary_table = pd.DataFrame(columns = ['Feature name'], data = train_data_X.columns.values)
summary_table['Coefficients'] = np.transpose(reg_1.coef_)
summary_table.index = summary_table.index + 1
summary_table.loc[0] = ['Intercept', reg_1.intercept_[0]]
summary_table = summary_table.sort_index()
p_values = reg_1.p_values
p_values = np.append(np.nan, np.array(p_values))
summary_table['p_values'] = p_values
summary_table
summary_table[summary_table['p_values'] < 0.05]
# Calculates the predicted probability values for the dependent variable (targets)
# based on the values of the independent variables (inputs) supplied as an argument.
Y_hat_test_proba = reg_1.model.predict_proba(train_data_X)[: ][: , 1]
Y_hat_test_proba
pd.options.display.max_rows = 15
df_actual_predicted = pd.concat([train_data_Y, pd.DataFrame(Y_hat_test_proba)], axis = 1)
df_actual_predicted.columns = ['Y_test', 'Y_hat_test_proba']
...
# Calculates the predicted probability values for the dependent variable (targets)
# based on the values of the independent variables (inputs) supplied as an argument.
Y_hat_test_proba = reg_1.model.predict_proba(test_data_X)[: ][: , 1]
Y_hat_test_proba
pd.options.display.max_rows = 15
df_actual_predicted = pd.concat([test_data_Y, pd.DataFrame(Y_hat_test_proba)], axis = 1)
df_actual_predicted.columns = ['Y_test', 'Y_hat_test_proba']
...
# Calculates the predicted probability values for the dependent variable (targets)
# based on the values of the independent variables (inputs) supplied as an argument.
Y_hat_test_proba = reg_2.model.predict_proba(train_data_X)[: ][: , 1]
Y_hat_test_proba
pd.options.display.max_rows = 15
df_actual_predicted = pd.concat([train_data_Y, pd.DataFrame(Y_hat_test_proba)], axis = 1)
df_actual_predicted.columns = ['Y_test', 'Y_hat_test_proba']
...
# Calculates the predicted probability values for the dependent variable (targets)
# based on the values of the independent variables (inputs) supplied as an argument.
Y_hat_test_proba = reg_2.model.predict_proba(test_data_X)[: ][: , 1]
Y_hat_test_proba
pd.options.display.max_rows = 15
df_actual_predicted = pd.concat([test_data_Y, pd.DataFrame(Y_hat_test_proba)], axis = 1)
df_actual_predicted.columns = ['Y_test', 'Y_hat_test_proba']
