...
for week in driver_sparse['week'].unique()[:]:
    ...
    current_past = driver_sparse[driver_sparse['week'] <= week][driver_sparse['week'] + pd.Timedelta(days = 7) >= week]
...
driver_final = driver_final[driver_final['week'] != latest_week].reset_index(drop = True)
...
driver_data_final = driver_data_self.merge(driver_final, how = 'left', on = ['week', 'driver_id'])
driver_data_self[driver_data_self['not_churn'] == 1]['not_churn_1w'].mean()
driver_data_final.isna().sum().sum()
def segment(sum_sh):
    if sum_sh < 10:
        return 'Part-time'
    if sum_sh < 20:
...
driver_data_final['segment'] = driver_data_final['sum_sh'].progress_apply(lambda x: segment(x))
driver_data_final[driver_data_final['driver_id'] == 342]
driver_data_final['week'].min()
# Split Train - Test
driver_data_final = driver_data_final.query('week > first_week')
del driver_data_final['first_week']
latest_week = driver_data_final['week'].max()
driver_data_test = driver_data_final[driver_data_final['week'] == latest_week].reset_index(drop = True)
driver_data_train = driver_data_final[driver_data_final['week'] != latest_week].reset_index(drop = True)
...
def plot_by_woe(df_WoE, rotation_of_x_axis_labels = 0, figsize = (10, 5)):
    ...
    plt.show()
discrete_cols = ['segment', 'not_churn']
...
selected_discrete_cols = ['segment','not_churn']
...
draw.to_csv('logistic_2w.csv', index = False)
...
'''
draw = con.execute(query).df()
draw.to_csv('rf_2w.csv', index = False)
fig = make_subplots(specs = [[{'secondary_y' : True}]])
fig.add_trace(
    pgo.Scatter(x = draw['week'], y = draw['churn_actual'],
                name = 'Actual Churn Rate', marker = dict(color = '#1068B3'),
