...
for week in driver_sparse['week'].unique()[:]:
    ...
    current = current.merge(current_past, how = 'left', on = 'driver_id').groupby(['week', 'driver_id', 'first_week']).mean().reset_index()
...
driver_data['week'].unique()[:2]
# Split Train - Test
driver_final = driver_final[driver_final['first_week'] + pd.Timedelta(days = 7) < driver_final['week']]
del driver_final['first_week']
...
driver_data_train[driver_data_train['driver_id'] == 342]
driver_data_train.describe()
...
def plot_by_woe(df_WoE, rotation_of_x_axis_labels = 0, figsize = (10, 5)):
    ...
    plt.show()
discrete_cols = ['segment', 'not_churn','not_churn_pre_1w']
...
selected_discrete_cols = ['segment','not_churn','not_churn_pre_1w']
...
df_continuous
woe_continuous_best_class_count(driver_data_train, 'driver_cancel')[1]
...
draw.to_csv('logistic_3w.csv', index = False)
...
'''
draw = con.execute(query).df()
draw.to_csv('rf_3w.csv', index = False)
fig = make_subplots(specs = [[{'secondary_y' : True}]])
fig.add_trace(
    pgo.Scatter(x = draw['week'], y = draw['churn_actual'],
                name = 'Actual Churn Rate', marker = dict(color = '#1068B3'),
